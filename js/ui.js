...
document.addEventListener("DOMContentLoaded", function(){
    ...
    /* input file button custom & output text field */
    
    const inputFileEl = document.querySelectorAll(".input-file");
    Array.prototype.forEach.call(inputFileEl, function(el, i) {
        el.querySelector("input[type='file']").addEventListener("change", function(e) {
            const fileArray = [];
            Array.prototype.push.apply(fileArray, e.target.files); 
            Array.prototype.forEach.call(fileArray, function(file, i) {
                el.childNodes[1].value = file.name;
            });
        });
    });

    /* input file multiple custom & file list delete */

    if(document.querySelector('.input-file--custom')) {
        const inputFileEl = document.querySelector('.input-file--custom input');
        const fileList = document.querySelector('.file-list');
        const fileBtn = document.querySelector('.input-file--custom .btn-input');
        const fileArray = [];
            ...
            function handleFiles() {
                ...
                // Array.isArray(inputFile); 배열인지 확인하기
